...
final class FaviconFetcher: NSObject, XMLParserDelegate {
  ...
  private static let userDefaults = UserDefaults(suiteName: "favicon")
    ...
    queue.async {
      self.parseHTMLForFavicons(url) { (icons, uri) in
        if !icons.isEmpty {
          ...
          userDefaults?.set(icons, forKey: uri)
        }
  ...
  fileprivate static func parseHTMLForFavicons(_ url: String, callback: @escaping ([String], String) -> Void) {
    ...
    if let icons = userDefaults?.array(forKey: uri.absoluteString) {
      debugPrint("fetchFromCache", uri, icons)
      return callback(icons as! [String], uri.absoluteString)
    }
