...
object FaviconFetcher {
  ...
  suspend fun getForURL(url: String?, preferences: SharedPreferences): List<String> =
    withContext(Dispatchers.IO) {
      try {
        if (url == null) return@withContext emptyList()

        val baseUrl = URL(removeQueryAndFragment(url))
        preferences.getString(baseUrl.toString(), null)?.let {
        ...
        return@withContext connection.inputStream.use { input ->
          ...
          if (!nonNullIcons.isEmpty()) {
            preferences.edit {
              println("saveToCache $baseUrl $nonNullIcons")
              putString(baseUrl.toString(), nonNullIcons.first())
            }
          }
  ...
  fun stableColorFrom(text: String): Int {
    ...
    val hex =
      DefaultFaviconBackgroundColors[hash.absoluteValue % DefaultFaviconBackgroundColors.size]
